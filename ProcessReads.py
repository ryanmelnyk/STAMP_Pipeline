...
def trim_tn(f,outdir):
	cmds = "scythe -a {} {}.filtered.fq -o {}.trimmed.fq -q sanger".format(os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])),"conserved_region.fna"),os.path.join(outdir,"filtered",f.split(".")[0]),os.path.join(outdir,"trimmed",f.split(".")[0]))
...
def uclust(f,outdir):
	cmds = "usearch --minlen 30 -cluster {} -uc {} -id 0.8".format(os.path.join(outdir,"unique",f.split(".")[0]+".unique.fa"),os.path.join(outdir,"uc",f.split(".")[0]+".uc"))
	proc = subprocess.Popen(cmds.split())
	proc.wait()
	return
...
def main():
	...
	try:
		...
		pass
	for f in ["raw","filtered","trimmed","exact","barcodes","unique","uc","clusters"]:
		try:
